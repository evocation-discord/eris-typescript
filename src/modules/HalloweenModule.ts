...
export default class HalloweenModule extends Module {
  ...
  async onMessage(message: Discord.Message): Promise<Discord.Message> {
    const mainGuild = message.client.guilds.resolve(env.MAIN_GUILD_ID);
    if (message.channel.id !== halloweenChannel) return;
    if (mainGuild.members.resolve(message.author.id).roles.cache.some((role) => [env.ROLES.ADMINISTRATORS, env.ROLES.LEAD_ADMINISTRATORS].includes(role.id))) return;
