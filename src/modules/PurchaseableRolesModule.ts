import { monitor, Module, ErisClient, MAIN_GUILD_ID, ROLES, command, inhibitors, strings, CommandCategories, commandDescriptions, messageLinkRegex, errorMessage, PV } from "@lib/utils";
...
export default class PurchaseableRolesModule extends Module {
  ...
  async muse(message: Message): PV<void> {
  ...
  async cancel(message: Message, user: User): PV<Message> {
  ...
  async educateme(message: Message): PV<void> {
