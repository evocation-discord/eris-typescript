    ...
    rolemanagement: {
      dethrone: {
        ...
        log: (executor: User, members: { member: GuildMember, roles: Role[]}[]) => {
          const strArray = [`\`[${timeFormatter()}]\` ${emotes.uncategorised.leave} **${members.length}** member${members.length === 1 ? "": "s"} was/were dethroned by ${executor} (\`${executor.id}\`).`];
      ...
      crown: {
        ...
        log: (executor: User, members: { member: GuildMember, roles: Role[]}[]) => {
          const strArray = [`\`[${timeFormatter()}]\` ${emotes.uncategorised.enter} **${members.length}** member${members.length === 1 ? "": "s"} was/were crowned by ${executor} (\`${executor.id}\`).`];
          for (const { member, roles } of members) {
            strArray.push(`${member.user} (\`${member.user.id}\`): ${roles.map(r => `**${r}**`).join(", ")}`);
          }
          return strArray.join("\n");
