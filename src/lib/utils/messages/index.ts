...
export const strings = {
  ...
  modules: {
    ...
    logging: {
      anonymisedAudit: (cmdTrigger: string, stringArgs: string[]) => `\`[${timeFormatter()}] **\`[ANONYMISED]\`** ${emotes.logging.anonymisedaudit} \`Command \`${cmdTrigger}\` was performed${stringArgs.length > 0 ? ` with args: \`${stringArgs.join(" ")}\`` : ""}. No further information is available.`,
